...
PowerIn = 2.0;                                                              % Power in W           
VoltageIn = sqrt(PowerIn*73);
...
for p=1:nunkns
    ...
    if p==matchpnt
        vmvector(p,1)=VoltageIn;                                                          % ******* Antenna feed voltage is 1 volt
    else
    vmvector(p,1)=0;
    end
end
...
for kk=1:1
    ...
    for i=1:ndx
        ...
        for j=1:ndy
            yd(j)=ystart+(j-1)*dy;
            ydj=yd(j);
            Distm(j)= yd(j)*wavelength;
            for k=1:ndz
                nfield_points=nfield_points+1;
                zd(k)=zstart+(k-1)*dz;
                zdk=zd(k);
                %Distm(k)= zd(k)*wavelength;
                ...
                % efieldx=efieldnear(rx,ry,rz,xdi,ydj,zdk,solvector,wk,rad,factor,delta,wavelength,1,0,0,nunkns);

                % efieldy=efieldnear(rx,ry,rz,xdi,ydj,zdk,solvector,wk,rad,factor,delta,wavelength,0,1,0,nunkns);
...
% % % 
plot(Distm(:),nearfield(1,:),'k-')
hold on
grid on

title(['Z-component of Electric Field for Half-wavelegh dipole @',num2str(freq/1e6),'MHz'])
xlabel('Distance (m)')
